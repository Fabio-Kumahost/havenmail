...
### Verifiziert: vollständiger Installer-Durchlauf auf Debian 13 / Dovecot 2.4
Nach den parallel entwickelten Dovecot-2.4-Kompatibilitätsfixes (Postfix-Pfade, Dovecot-Listener-Syntax, TLS-Einstellungen, Cipher-Präferenz, `ssl_server_cert_file`-Pfad) end-to-end in einer Debian-13-Container-Simulation (systemd als PID 1) bestätigt: kompletter `curl | bash --unattended`-Ablauf inkl. Rust-/Frontend-Build, Config-Rendering mit den Dovecot-2.4-Patches, alle 7 Dienste (`postfix`, `dovecot`, `rspamd`, `clamav-daemon`, `nginx`, `fail2ban`, `havenmail-api`) aktiv, Admin-Bootstrap und erfolgreicher Login (HTTP 200). TLS-Ausstellung war dabei simuliert (kein echtes DNS im Container) — auf der echten VM des Nutzers mit echtem DNS noch zu bestätigen. Kleiner kosmetischer Fund dabei: nginx auf Debian 13 warnt (nicht fehlerhaft) vor der veralteten `listen ... http2`-Syntax — funktioniert weiterhin, aber ein Update auf die neue `http2 on;`-Direktive (mit Versions-Unterscheidung wie bei den Dovecot-Patches) wäre eine spätere Politur.

### Behoben — kritisch, live auf der echten VM des Nutzers gefunden (2. Fund nach dem BASH_SOURCE-Fix)
- **`curl | sudo bash`: interaktive `read -rp`-Abfragen lasen versehentlich von der bereits verbrauchten stdin-Pipe statt vom Terminal.** Bei `curl URL | sudo bash` ist stdin (fd 0) durchgehend an die Pipe von `curl` gebunden — auch nach dem Self-Bootstrap-`exec`. Sobald `curl` fertig gesendet hat, liefert diese Pipe EOF bzw. (je nach Timing) noch unverarbeitete Skript-Bytes, die dann fälschlich als Nutzereingabe interpretiert wurden. Real aufgetreten: der Mail-Hostname wurde mit Skript-/Kommentarresten kontaminiert, was den gerenderten nginx-vhost mit `directive "server_name" is not terminated by ";"` zum Absturz brachte. Fix: `install.sh` öffnet `/dev/tty` explizit auf fd 3 und liest alle vier interaktiven Abfragen (`read -r -p ... <&3`) davon; ohne verfügbares TTY (z. B. in einer Nicht-interaktiven CI-Pipe) bricht es jetzt mit einer klaren Fehlermeldung ab, die auf `--unattended` verweist, statt still falsche Werte zu übernehmen.
- `havenmail_configure_postgres` verließ sich implizit darauf, dass `apt-get install postgresql` den Dienst automatisch startet — kein Fehlschlag auf einer normalen VM, aber keine Garantie (in einem Container mit `policy-rc.d`-Sperre real fehlgeschlagen: "connection to server on socket ... failed"). Jetzt mit `systemctl enable --now postgresql` defensiv abgesichert (idempotent, harmlos falls schon aktiv).
